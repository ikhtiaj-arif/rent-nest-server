...
const createPayment = async (payload, tenantId) => {
    ...
    if (rentalRequest.tenantId !== tenantId) {
        throw new Error("Forbidden. You do not have permission to pay for this rental");
    }
...
const handleStripeWebhook = async (rawBody, signature, webhookSecret) => {
    let event;
    try {
...
const handlePaymentSuccess = async (sessionId, metadata) => {
...
const handlePaymentFailure = async (sessionId) => {
    const payment = await prisma.payment.findUnique({
        where: { stripePaymentIntentId: sessionId },
    });
    if (!payment) {
...
const getUserPayments = async (tenantId, role, query) => {
...
const getPaymentById = async (paymentId, tenantId, role) => {
    const payment = await prisma.payment.findUnique({
        where: { id: paymentId },
        include: {
            rentalRequest: {
