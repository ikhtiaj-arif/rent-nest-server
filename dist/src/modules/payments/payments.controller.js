...
const confirmPayment = catchAsync(async (req, res, next) => {
    ...
    const result = await paymentService.handleStripeWebhook(event, signature, config.stripe_webhook_secret);
...
const getPayments = catchAsync(async (req, res, next) => {
    ...
    const result = await paymentService.getUserPayments(tenantId, role, req.query);
    sendResponse(res, {
        success: true,
        statusCode: httpStatus.OK,
