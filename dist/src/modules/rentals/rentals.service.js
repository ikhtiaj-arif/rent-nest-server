...
const createRentals = async (payload, tenantId) => {
...
const getRentalsOnPropertyForLandlord = async (landlordId, query) => {
...
const getRentalsById = async (rentalId) => {
...
const getUserRentalRequestsDB = async (tenantId, query) => {
    const limit = query.limit ? Number(query.limit) : 10;
    const page = query.page ? Number(query.page) : 1;
    const skip = (page - 1) * limit;
    const sortBy = query.sortBy || "createdAt";
...
const approveRentalRequest = async (rentalRequestId, landlordId, status) => {
    // validate status
    // find rental
